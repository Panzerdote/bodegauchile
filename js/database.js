...
const DB = {
    async getSecciones() {
        const { data, error } = await supabaseClient.from('secciones').select('*').order('seccion').order('anaquel');
        if (error) { console.error('Error al obtener secciones:', error); throw error; }
        return data || [];
    },
    ...
    async getInventario() {
        const { data, error } = await supabaseClient.from('inventario').select('*').order('seccion').order('anaquel');
        if (error) { console.error('Error al obtener inventario:', error); throw error; }
        return data || [];
    },
    ...
    async getMovimientos(limit = 50) {
        const { data, error } = await supabaseClient.from('movimientos').select('*').order('fecha', { ascending: false }).limit(limit);
        if (error) throw error;
        return data || [];
    },

    async getTodosMovimientos() {
        const { data, error } = await supabaseClient.from('movimientos').select('*').order('fecha', { ascending: false }).limit(1000);
        if (error) throw error;
        return data || [];
    },
    ...
    async getConfig() {
        const { data, error } = await supabaseClient.from('configuracion').select('*').order('id').limit(1).single();
        if (error) return { porcentaje_critico: 20, dias_vencimiento: 30 };
        return data;
    },

    async getUnidadesMedida() {
        const { data, error } = await supabaseClient.from('unidades_medida').select('*').order('nombre');
        if (error) throw error;
        return data || [];
    },
    ...
    async buscarInsumosNombre(busqueda) {
        const { data, error } = await supabaseClient.from('inventario').select('nombre, unidad').ilike('nombre', `%${busqueda}%`).order('nombre').limit(10);
        if (error) throw error;
        const unicos = []; const nombres = new Set();
        (data || []).forEach(item => { if (!nombres.has(item.nombre.toLowerCase())) { nombres.add(item.nombre.toLowerCase()); unicos.push(item); } });
        return unicos;
    },
