function ahoraChile() { const a = new Date(); const d = (-4*60 - a.getTimezoneOffset()) * 60 * 1000; return new Date(a.getTime() + d); }
...
const DB = {
    ...
    async addInventarioItem(item) {
        const bodega = window.currentBodega || 'BODEGA';
        const { data, error } = await supabaseClient.from('inventario').insert([{ nombre: item.nombre ? item.nombre.toUpperCase() : null, seccion: item.seccion, anaquel: item.anaquel, stock: item.stock || 0, unidad: item.unidad ? item.unidad.toUpperCase() : null, lote: item.lote ? item.lote.toUpperCase() : null, vencimiento: item.vencimiento || null, comentarios: item.comentarios ? item.comentarios.toUpperCase() : null, bodega }]).select();
        if (error) throw error; return data[0];
    },
    async updateInventarioItem(id, updates) {
        if (updates.nombre) updates.nombre = updates.nombre.toUpperCase(); if (updates.unidad) updates.unidad = updates.unidad.toUpperCase();
        if (updates.lote) updates.lote = updates.lote.toUpperCase(); if (updates.comentarios) updates.comentarios = updates.comentarios.toUpperCase();
    ...
    async addMovimiento(movimiento) {
        ...
        const { data, error } = await supabaseClient.from('movimientos').insert([{ fecha: ahoraChile().toISOString(), tipo: movimiento.tipo, insumo: movimiento.insumo ? movimiento.insumo.toUpperCase() : null, cantidad: movimiento.cantidad || 0, stock_anterior: movimiento.stock_anterior !== undefined ? movimiento.stock_anterior : null, stock_nuevo: movimiento.stock_nuevo !== undefined ? movimiento.stock_nuevo : null, anaquel: movimiento.anaquel ? movimiento.anaquel.toUpperCase() : null, comentarios: movimiento.comentarios ? movimiento.comentarios.toUpperCase() : null, usuario, bodega }]).select();
        if (error) throw error; return data[0];
    },
    ...
    async buscarInsumosNombre(busqueda) {
        const bodega = window.currentBodega || 'BODEGA';
        const { data, error } = await supabaseClient.from('inventario').select('nombre, unidad').ilike('nombre', `%${busqueda}%`).eq('bodega', bodega).order('nombre').limit(10);
        if (error) throw error; const u = []; const n = new Set(); (data||[]).forEach(i => { if (!n.has(i.nombre.toLowerCase())) { n.add(i.nombre.toLowerCase()); u.push(i); } }); return u;
    },

    async procesarIngreso(nombre, seccion, anaquel, cantidad, unidad, lote, vencimiento, comentarios) {
        try {
            const bodega = window.currentBodega || 'BODEGA'; const nombreUpper = nombre.toUpperCase();
            const { data: existentes } = await supabaseClient.from('inventario').select('*').ilike('nombre', nombreUpper).eq('anaquel', anaquel).eq('bodega', bodega);
            let itemId, sa = 0, sn = cantidad;
            if (existentes && existentes.length > 0) { const item = existentes[0]; itemId = item.id; sa = item.stock; sn = sa + cantidad; const u = { stock: sn }; if (lote) u.lote = lote.toUpperCase(); if (vencimiento) u.vencimiento = vencimiento; if (comentarios) u.comentarios = comentarios.toUpperCase(); if (unidad) u.unidad = unidad.toUpperCase(); await this.updateInventarioItem(itemId, u); }
            else { const nuevo = await this.addInventarioItem({ nombre: nombreUpper, seccion, anaquel, stock: cantidad, unidad: unidad?unidad.toUpperCase():null, lote: lote?lote.toUpperCase():null, vencimiento: vencimiento||null, comentarios: comentarios?comentarios.toUpperCase():null }); itemId = nuevo.id; }
    ...
    async procesarSalida(itemId, cantidad, comentarios) {
        try {
            const item = await this.getInventarioItem(itemId); if (!item) throw new Error('INSUMO NO ENCONTRADO');
