const App = {
    ...
    renderDashboard() {
        ...
        document.getElementById('vencimientos-proximos').textContent = inventario.filter(i => { if (!i.vencimiento) return false; const v = new Date(i.vencimiento + 'T00:00:00'); return v >= hoy && v <= lim; }).length;
    ...
    renderAlertas(criticos) {
        ...
        const hoy = new Date(); const lim = new Date(hoy); lim.setDate(lim.getDate() + (config.dias_vencimiento || 30));
        const pv = inventario.filter(i => { if (!i.vencimiento) return false; const v = new Date(i.vencimiento + 'T00:00:00'); return v >= hoy && v <= lim; });
        const ids = new Set(criticos.map(i => i.id)); const pvf = pv.filter(i => !ids.has(i.id)); const total = criticos.length + pvf.length;
        ...
        criticos.forEach(item => { const v = item.vencimiento ? new Date(item.vencimiento + 'T00:00:00') : null; const venc = v && v < hoy; const vp = v && !venc && v <= lim; let ta = ''; if (venc) ta = '<span class="badge badge-danger">VENCIDO</span> <span class="badge badge-danger">CRÍTICO</span>'; else if (vp) ta = '<span class="badge badge-danger">CRÍTICO</span> <span class="badge badge-warning">POR VENCER</span>'; else ta = '<span class="badge badge-danger">CRÍTICO</span>'; h += `<tr class="stock-critical"><td><strong>${item.nombre}</strong></td><td class="text-center">${item.stock} ${item.unidad||''}</td><td class="text-center">${item.anaquel}</td><td class="text-center">${item.vencimiento||'N/A'}</td><td class="text-center">${ta}</td></tr>`; });
    ...
    async procesarIngreso() {
        ...
        try { await DB.procesarIngreso(nombre, seccion, anaquel, cantidad, unidad, lote, vencimiento, comentarios); UI.closeModal(); UI.showToast('INGRESO REGISTRADO', 'success'); await this.loadAllData(); this.renderDashboard(); } catch (e) { UI.showToast('ERROR: ' + e.message, 'error'); }
    },
    ...
    async crearNuevaSeccion() {
        const l = document.getElementById('nueva-seccion-letra').value.trim().toUpperCase(); const d = document.getElementById('nueva-seccion-descripcion').value.trim().toUpperCase();
        ...
        try { for (let i = 1; i <= c; i++) await DB.addSeccion(l, d, String(i)); await DB.addMovimiento({ tipo: 'CREACION_SECCION', insumo: `SECCIÓN ${l}`, cantidad: c }); await this.loadAllData(); this.mostrarContenidoSecciones(); this.renderDashboard(); UI.showToast('SECCIÓN CREADA', 'success'); } catch (e) { UI.showToast('ERROR', 'error'); }
    },
    ...
    exportarExcel() { const { inventario } = this.state; if (!inventario.length) { UI.showToast('SIN DATOS', 'warning'); return; } let csv = 'ID;NOMBRE;ANAQUEL;STOCK;UNIDAD;LOTE;VENCIMIENTO;COMENTARIOS\n'; inventario.forEach(i => { csv += `${i.id};"${i.nombre}";${i.anaquel};${i.stock};${i.unidad||''};${i.lote||''};${i.vencimiento||''};"${i.comentarios||''}"\n`; }); const b = new Blob(['\uFEFF'+csv], {type:'text/csv;charset=utf-8;'}); const u = URL.createObjectURL(b); const a = document.createElement('a'); a.href = u; a.download = `INVENTARIO_${window.currentBodega}_${new Date().toISOString().split('T')[0]}.csv`; a.click(); URL.revokeObjectURL(u); UI.showToast('EXPORTADO', 'success'); },
